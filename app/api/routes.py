...
async def explore_data_open(json_data: str = Form(...)):
    """Form-based endpoint — opens directly in new tab, no CSP inheritance."""
    try:
...
async def chart_data_open(json_data: str = Form(...), chart_type: str = Form("auto")):
    """Form-based endpoint — opens directly in new tab, no CSP inheritance."""
    try:
        data = json.loads(json_data)
        if chart_type == "auto":
            ...
            html = generate_typed_chart_html(data, chart_type)
        return HTMLResponse(content=html)
    except Exception as e:
        raise HTTPException(500, f"Erro ao gerar gráfico: {str(e)}")
